...
def count_gini(thresholds_local, sort_feature_vector, target_vector):
    ...
    R = target_vector
    answer = -np.sum(Rl, axis=1) / np.sum(R) * count_H(Rl, target_vector) - np.sum(Rr, axis=1) / np.sum(R) * count_H(Rr, target_vector)
    return answer
...
class DecisionTree(BaseEstimator):
    ...
    def _fit_node(self, sub_X, sub_y, node):
        ...
        if  hasattr(self, '_min_samples_split') and self._min_samples_split is not None and len(sub_y) <= self._min_samples_split:  # 4
            node["type"] = "terminal"
